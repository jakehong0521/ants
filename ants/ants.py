    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'is_queen') and self.ant.is_queen:
                    return
        ...
        if not hasattr(insect, 'is_queen') or not insect.is_queen:
            insect.place = None
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        return not not self.place.ant and self.place.ant.blocks_path
        # END Problem 6A
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while not room == hive:
            if not room.bees == []:
                if move in range(self.min_range, self.max_range):
                    return random_or_none(room.bees)
            room = room.entrance
            move += 1
...
class BodyguardAnt(Ant):
    ...
    def __init__(self):
        Ant.__init__(self, 2)
        self.ant = None  # The Ant hidden in this No
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.is_queen:
            ...
            room = room.exit
            while not room == None:
                if room.ant and room.ant.doubled == False:
                    room.ant.damage = room.ant.damage * 2
                    room.ant.doubled = True
                if room.ant and room.ant.container and room.ant.ant != None and room.ant.ant.doubled == False: 
                    room.ant.ant.damage = room.ant.ant.damage * 2
                    room.ant.ant.doubled = True
                room = room.exit
        else:
